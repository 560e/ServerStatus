...
# 提示用户输入密码
read -p "请输入服务器密码: " server_password
...
# 1. 在当前目录创建一个文件，文件名为：sap560-clent.py
touch sap560-clent.py

# 2. 生成sap560-clent.py的内容
cat <<EOL > sap560-clent.py
#!/usr/bin/env python3
# coding: utf-8
...
# 将当前目录下的 sap560-clent.py 复制到目录：/usr/local/bin/
yes | cp -f sap560-clent.py /usr/local/bin/

# 在当前目录创建一个空的服务文件，文件名为：sap560-clent.service
touch sap560-clent.service

# 创建服务配置文件 sap560-clent.service
cat <<EOL > sap560-clent.service
[Unit]
Description=sap560-clent server

[Service]
Type=simple
ExecStart=/usr/bin/python3 /usr/local/bin/sap560-clent.py </dev/null >/dev/null 2>&1
Restart=always
...
# 将服务文件移动到系统服务目录
yes | cp -f sap560-clent.service /etc/systemd/system/

# 将 sap560-clent.service 服务设置为开机自动启动
systemctl enable sap560-clent.service

# 重载服务
systemctl daemon-reload

# 启动服务使其生效
systemctl start sap560-clent.service
